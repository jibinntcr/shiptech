...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['mouEditBTN'])) {
        if (($_FILES["file"]["error"]) == 4) {
            ...
            $name = $_POST['EditName'];
            $year = $_POST['EditYear'];
            $sql = "UPDATE  placementBrochure SET name='$name',year='$year',image='$photo' where id='$id'";
            $query = $dbh->prepare($sql);
            $result = $query->execute();
            if ($query->rowCount() > 0) {
                echo '<script>alert("Success")</script>';
                echo '<script>window.location = "placement-brochure.php";</script>';
            } else {
...
<head>
    ...
    <title>Edit Placement Brochure | Ship Technology Admin</title>
    <!-- plugins:css -->
                ...
                <div class="content-wrapper">
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h4 class="card-title">Edit Placement Brochure</h4>
                                <form class="forms-sample" enctype="multipart/form-data" method="POST">
                                    <?php
                                        $id = $_GET['id'];
                                        $sql = "SELECT * from placementBrochure where id=$id ";
                                        $query = $dbh->prepare($sql);
                                        $query->execute();
                                    ...
                                    <div class="form-group">
                                        <label for="name"><?php echo ($userArr[0]->image); ?></label>
                                        <input type="file" class="form-control" id="file" name="file" accept=".pdf">
                                    </div>
                                    <button type="submit" class="btn btn-primary mr-2" name="mouEditBTN"
                                        id="mouEditBTN">Save</button>

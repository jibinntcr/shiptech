...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['mouEditBTN'])) {
        if (($_FILES["file"]["error"]) == 4) {
            ...
            $sql = "UPDATE  mous SET name=' $name',year='$year',description='$content',image='$photo' where id='$id'";
            $query = $dbh->prepare($sql);
            $result = $query->execute();
            if ($query->rowCount() > 0) {
                echo '<script>alert("Success")</script>';
                echo '<script>window.location = "mou.php";</script>';
            } else {
...
?>
    ...
    <head>
        ...
        <title>Edit MoU | Ship Technology Admin</title>
        <!-- plugins:css -->
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <form class="forms-sample" enctype="multipart/form-data" method="POST">
                                        <?php
                                        $id = $_GET['id'];
                                        $sql = "SELECT * from mous where id=$id ";
                                        $query = $dbh->prepare($sql);
                                        $query->execute();
                                        ...
                                        ?>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" id="EditContent" name="EditContent" rows="4"><?php echo htmlentities($userArr[0]->description); ?></textarea>
                                            </div>
                                            <div class="form-group">
                                                <input type="file" id="file" name="file" accept="image/gif, image/png, image/jpg, image/jpeg">
                                            </div>
                                            <button type="submit" class="btn btn-primary mr-2" name="mouEditBTN" id="mouEditBTN">Save</button>
